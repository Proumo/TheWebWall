...
function amigosUsuario(){
    $("#folksContainer").load("controller/mostrarAmigos.php");
}
function editarFotoPerfil(){
    $("#postInfoContent").html("").load("controller/editarPerfilForm.php");
...
function searchPeople(){
    ...
    $.ajax({
        ...
        success:function(html){
            if($("#postInfo").is(":hidden")){
            $("#postInfoContent").html(html);
            $("#postInfo").show("slow")
    }else{
            $("#postInfo").hide("slow");
            $("#postInfoContent").html("").html(html);
            $("#postInfo").show("slow");
    }}
    });
...
function carregar(x,y){
    ...
    //$.cre(x,y);
    //var id="#"+x+"-"+y;
    //var $target = $("#demo2_viewport").find(id);
    //$('#demo2_viewport').scrollTo( $target , 300, {onAfter:function(){$.update();}});
}

function postsUsuario(){
    $("#meusPosts").load("controller/mostrarPostsUsuario.php");
}
function amigosUsuario(){
    $("#folksContainer").load("controller/mostrarAmigos.php");
}
function navegar(x,y,id){
   if(id){
    ...
    $.ajax({
            ...
            //var target1=$(id1);
            //var target2=$(id);
   // if(target1.offset()===null||target1.offset()===undefined){
    //    alert(x2);
    //    alert(y2);
    //    $.create(x2,y2);
    //}
    //if(target2.offset()===null||target2.offset()===undefined){
    //    $.create(x,y);
    //}
        //$('#demo2_viewport').scrollTo( target1, 2000, {onAfter:function(){$.update();
        //$('#demo2_viewport').scrollTo( target2, 2000, {onAfter:function(){$.update();}});    
        //}});
        return;
...
function removerComentario(id,idPost){
    ...
    $.post("controller/removerComentario.php",{"id":id},function(){
        ...
        $("#postInfoContent").load("controller/mostrarInfo.php?id="+idPost);
        }, null);
    
}
function showInformation(id){
    
    $("#postInfoContent").html("").load("controller/mostrarInfo.php?id="+id);
    $("#postInfo").show("slow");
...
function enviarComentario(id){
    ...
    $.ajax({
        ...
        dataType:"text",
        success:function(){
             $("#postInfoContent").load("controller/mostrarInfo.php?id="+id);
